...
## [1 前言](ch01.md#1-前言)
  + [1.1 小型嵌入式系统中的多任务处理](ch01.md#11-小型嵌入式系统中的多任务处理)
    + [1.1.1 关于 FreeRTOS 内核](ch01.md#111-关于-FreeRTOS-内核)
    + [1.1.2 价值主张](ch01.md#112-价值主张)
    + [1.1.3 关于术语的说明](ch01.md#113-关于术语的说明)
    + [1.1.4 为什么要使用 RTOS？](ch01.md#114-为什么要使用-RTOS？)
    + [1.1.5 FreeRTOS 内核特性](ch01.md#115-FreeRTOS-内核特性)
    + [1.1.6 许可以及 FreeRTOS，OpenRTOS 和 SafeRTOS 家族](ch01.md#116-许可以及-FreeRTOS，OpenRTOS-和-SafeRTOS-家族)
  + [1.2 包含的源文件和项目](ch01.md#12-包含的源文件和项目)
    + [1.2.1 获取随附本书的示例](ch01.md#121-获取随附本书的示例)

## [2 FreeRTOS 内核发行](ch02.md#2-FreeRTOS-内核发行)
  + [2.1 概述](ch02.md#21-概述)
  + [2.2 了解 FreeRTOS 的发行](ch02.md#22-了解-FreeRTOS-的发行)
    + [2.2.1 定义: FreeRTOS 移植](ch02.md#221-定义:-FreeRTOS-移植)
    + [2.2.2 构建 FreeRTOS](ch02.md#222-构建-FreeRTOS)
    + [2.2.3 FreeRTOSConfig.h](ch02.md#223-freertosconfigh)
    + [2.2.4 官方发行版](ch02.md#224-官方发行版)
    + [2.2.5 所有移植共用的 FreeRTOS 源文件](ch02.md#225-所有移植共用的-FreeRTOS-源文件)
    + [2.2.6 特定移植的 FreeRTOS 源文件](ch02.md#226-特定移植的-FreeRTOS-源文件)
    + [2.2.7 引用路径](ch02.md#227-引用路径)
    + [2.2.8 头文件](ch02.md#228-头文件)
  + [2.3 示例应用程序](ch02.md#23-示例应用程序)
  + [2.4 创建一个 FreeRTOS 项目](ch02.md#24-创建一个-FreeRTOS-项目)
    + [2.4.1 改造一个提供示例项目](ch02.md#241-改造一个提供示例项目)
    + [2.4.2 从头创建一个新的项目](ch02.md#242-从头创建一个新的项目)
...
## [3 堆内存管理](ch03.md#3-堆内存管理)
  + [3.1 简介](ch03.md#31-简介)
    + [3.1.1 前提条件](ch03.md#311-前提条件)
    + [3.1.2 范围](ch03.md#312-范围)
    + [3.1.3 动态内存分配和静态内存分配的切换](ch03.md#313-动态内存分配和静态内存分配的切换)
    + [3.1.4 使用动态内存分配](ch03.md#314-使用动态内存分配)
    + [3.1.5 动态分配内存的选项](ch03.md#315-动态分配内存的选项)
  + [3.2 示例内存分配方案](ch03.md#32-示例内存分配方案)
    ...
    + [3.2.6 初始化 heap_5: vPortDefineHeapRegions() API 函数](ch03.md#326-初始化-heap_5:-vPortDefineHeapRegions()-API-函数)
  + [3.3 堆相关的实用函数和宏](ch03.md#33-堆相关的实用函数和宏)
    + [3.3.1 定义堆起始地址](ch03.md#331-定义堆起始地址)
    + [3.3.2 xPortGetFreeHeapSize() API 函数](ch03.md#332-xPortGetFreeHeapSize-API-函数)
    + [3.3.3 xPortGetMinimumEverFreeHeapSize() API 函数](ch03.md#333-xportgetminimumeverfreeheapsize-API-函数)
    + [3.3.4 vPortGetHeapStats() API 函数](ch03.md#334-vportgetheapstats-API-函数)
    + [3.3.5 收集每个任务堆使用统计信息](ch03.md#335-收集每个任务堆使用统计信息)
    + [3.3.6 申请失败钩子函数](ch03.md#336-申请失败钩子函数)
    + [3.3.7 将任务栈放在快速内存中](ch03.md#337-将任务栈放在快速内存中)
  + [3.4 使用静态内存分配器](ch03.md#34-使用静态内存分配器)
    + [3.4.1 启用静态内存分配器](ch03.md#341-启用静态内存分配器)
...
## 图:
+ [图 2.1 FreeRTOS 发行版的目录结构](ch02.md#fig2.1)
+ [图 2.2 FreeRTOS 目录树中的核心源文件](ch02.md#fig2.2)
+ [图 2.3 FreeRTOS 目录树中移植特有的源文件](ch02.md#fig2.3)
+ [图 2.4 示例程序目录结构](ch02.md#fig2.4)
+ [图 3.1 每次创建任务时，从 heap_1 数组分配 RAM](ch03.md#fig3.1)
+ [图 3.2 每次创建或删除任务时，从 heap_2 数组分配或释放内存](ch03.md#fig3.2)
+ [图 3.3 从 heap_4 数组分配或释放内存](ch03.md#fig3.3)
...
## 清单:
+ [清单 2.1 main() 函数的模板](ch02.md#list2.1)
+ [清单 3.1 列表 3.1 vPortDefineHeapRegions() API 函数原型](ch03.md#list3.1)
+ [清单 3.2 HeapRegion_t 结构体](ch03.md#list3.2)
+ [清单 3.3 共同描述了三个内存块的全部内容的 HeapRegion_t 数组](ch03.md#list3.3)
+ [清单 3.4 描述所有 RAM2、所有 RAM 3、部分 RAM1 的 HeapRegion_t 结构体数组](ch03.md#list3.4)
+ [清单 3.5 使用 GCC 语法声明 heap_4 使用的数组，并将其放在 .my_heap 内存段](ch03.md#list3.5)
+ [清单 3.6 使用 IAR 语法声明 heap_4 使用的数组，并将数组放在绝对内存地址 0x20000000](ch03.md#list3.6)
...
+ [清单 3.11 申请失败函数名及原型](ch03.md#list3.11)
+ [清单 3.12 将 pvPortMallocStack() 和 vPortFreeStack() 宏映射成用户程序定义的内存分配器](ch03.md#list3.12)
+ [清单 3.13 vApplicationGetTimerTaskMemory 函数的典型实现](ch03.md#list3.13)
+ [清单 3.14 vApplicationGetIdleTaskMemory 函数的典型实现](ch03.md#list3.14)
...
## 表:
+ [表 1 项目中需要包含的 FreeRTOS 源文件](ch02.md#tbl1)
+ [表 2 TickType_t 数据类型和 configTICK_TYPE_WIDTH_IN_BITS 配置的关系](ch02.md#tbl2)
+ [表 3 宏前缀说明](ch02.md#tbl3)
+ [表 4 通用宏定义](ch02.md#tbl4)
